<?php
    ...
    require_once("../../settings/user_info.php");
...
$receiverGHID = mysql_prep($_GET['receiverGHID']);
$matchID = mysql_prep($_GET['matchID']);
...
// set variables
$dir_dest = (isset($_GET['dir']) ? $_GET['dir'] : POP_DIR_FULL);
$dir_pics = (isset($_GET['pics']) ? $_GET['pics'] : $dir_dest);

if (!$cli && !(isset($_SERVER['HTTP_X_FILE_NAME']) && isset($_SERVER['CONTENT_LENGTH']))) { //if not Ajax call, print the document title,css,e.t.c
?>
...
if ((isset($_POST['action']) ? $_POST['action'] : (isset($_GET['action']) ? $_GET['action'] : '')) == 'simple') {
    ...
    if ($handle->uploaded) {
       ...
       $handle->image_reflection_height = '40px';
        ...
        $handle->file_new_name_body = "POP".substr($receiverGHID,2)."M".$matchID."U".$_SESSION['user_id']."T".time(); 
        ...
        if ($handle->processed) {
            // $desc=mysql_prep($_POST['img_desc']);
            $stmt=$con->prepare("UPDATE matching set confirmed_by_sender = '1' , teller = ? where match_id=? ");
             $stmt->bind_param("ss",$handle->file_dst_name,$matchID);
             $stmt->execute();
    ...
    // ---------- IMAGE UPLOAD ----------
$j=json_decode($_POST['avatar_data']);
echo $_POST['avatar_data'];
    ...
    if ($handle->uploaded) {
        ...
        $handle->image_resize            = true;
        $handle->image_x           = "400px";
        $handle->image_y           = "400px";
        $handle->image_ratio_y           = true;
        $handle->image_ratio_x           = true;
       
        $handle->image_rotate          = $j->rotate;
       $handle->image_precrop            = $j->y." ".$j->x." ".$j->y;
